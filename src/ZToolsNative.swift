...
private var btnMoved = false                      // 是否检测到位移（超过阈值）
...
private var storedMouseDownLocation: CGPoint = .zero // 按下时的鼠标位置

// 左键长按位移阈值（5px，平方值避免 sqrt）
private let MOUSE_MOVE_THRESHOLD_SQ: Double = 25.0
...
func mouseEventTapHandler(_ proxy: CGEventTapProxy, _ type: CGEventType, _ event: CGEvent, _ userInfo: UnsafeMutableRawPointer?) -> Unmanaged<CGEvent>? {
    ...
    // ── 左键拖拽检测（位移超阈值则取消长按，恢复拖拽行为）──
    if mouseTargetIsLeft && type == .leftMouseDragged {
        mouseLock.lock()
        let isDown = btnIsDown
        let moved = btnMoved
        let downLocation = storedMouseDownLocation
    ...
    if isTargetDown {
        ...
        storedMouseDownLocation = event.location
        btnTimer?.cancel()

        if mouseIsLongPress {
            let timer = DispatchWorkItem {
                mouseLock.lock()
                guard btnIsDown && !btnMoved else {
                    mouseLock.unlock()
                    return
                }
    ...
    if isTargetUp {
        ...
        if mouseIsLongPress {
            if wasMoved {
                // 拖拽模式：mouseDown 已在拖拽检测时重放，直接放行 mouseUp
                return Unmanaged.passUnretained(event)
                ...
                // 未达到长按阈值，重放原始 mouseDown + mouseUp（恢复普通点击行为）
                if let downEvent = storedDown {
                    downEvent.setIntegerValueField(.eventSourceUserData, value: MOUSE_REPLAY_MARKER)
                    downEvent.post(tap: .cgSessionEventTap)
                }
...
public func stopMouseMonitor() {
    ...
    btnTimer?.cancel()
    btnTimer = nil
    // 如果有未完成的按下事件且尚未进入拖拽模式，重放它以恢复默认行为
    if btnIsDown && !btnMoved, let storedDown = storedMouseDownEvent {
